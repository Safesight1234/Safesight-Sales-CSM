/* ============================================================
   Historicals tab — restored working version
   ============================================================ */
(function () {
  ...
  function safe(data, y) {
    var pts = data && (data[y] || data[String(y)]);
    if (!Array.isArray(pts)) return [0, 0, 0, 0];
    return pts.map(function(v) { return (typeof v === 'number' && isFinite(v)) ? v : 0; });
  }

  function HistBlock({ title, data, cur, hotYear }) {
    if (!data) return <Card title={title}><div className="empty">No data.</div></Card>;
